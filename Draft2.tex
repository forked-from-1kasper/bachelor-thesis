...
Группу корней степени $n$ из~единицы, задаваемую уравнением $z^n = 1$, обозначим как $\sqrt[n]{1}$.
...
\begin{proof}
    ...
    Для~вектора $u \in \sqrt[\varepsilon_1]{1} \times \ldots \times \sqrt[\varepsilon_r]{1}$ рассмотрим множество $H_u = \{z \in (\torus)^n \ | \ \forall 1 \leq i \leq r{:}\ z^{b_i} = u_i\}$.
    Условие $\phi_\varepsilon(\phi_{\beta_2^{-1}}(z)) = 1$, очевидно, эквивалентно условию $\exists u \in \sqrt[\varepsilon_1]{1} \times \ldots \times \sqrt[\varepsilon_r]{1}{:}\ z \in H_u$.
    Ясно, что множества $H_u$ дизъюнктны, поэтому $\ker(\phi_{\beta})$ распадается в~дизъюнктное объединение:
    $$
        \ker(\phi_{\beta}) = \bigsqcup_u H_u.
    $$

    Векторов $u$ ровно $\Pi(\beta)$ штук, поэтому достаточно показать, что каждый $H_u$ связен.

    Поскольку $H_1 = \ker(\phi_{\delta \beta_2^{-1}})$, где $\delta = \diag^{k \times n}_r(1, \ldots, 1)$,
    и~$\Pi(\delta \beta_2^{-1}) = 1$, $H_1$, по~замечанию выше, связно.
...
\begin{proof}
    ...
    Отображение $\phi_{\delta \beta_2^{-1}} : \ker(\phi_\beta) \rightarrow \sqrt[\varepsilon_1]{1} \times \ldots \times \sqrt[\varepsilon_r]{1}$
    индуцирует инъективный гомоморфизм из~фактора. Помимо этого, поскольку все компонентны $H_u$ гомеоморфны (!!! ??? !!!), они все непусты.
    Это означает, что индуцированное отображение сюръективно. Таким образом, получаем изоморфизмы:
    $$
        \Z / \varepsilon_1 \Z \times \ldots \times \Z / \varepsilon_r \Z \cong \ker(\phi_\beta) / \ker(\phi_{\delta \beta_2^{-1}})
...
как компоненту связности, содержащую единицу группы. Из~доказанной теоремы следует, что $\Pi(H^\circ) = 1$.
