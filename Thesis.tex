...
\begin{proof}
    ...
    Копроизведение единственно, поэтому $T(a) + T(b) \cong T(a + b)$, например, посредством стрелки
    $\phi = \mathrm{rec}_{T(a), T(b)}(T(\mathrm{inl}_{a, b}), T(\mathrm{inr}_{a, b})) \in \mathrm{hom}_B(T(a) + T(b), T(a + b))$.
    Кроме того, $\phi \circ \mathrm{inl}_{T(a), T(b)} = T(\mathrm{inl}_{a, b})$
    и~$\phi \circ \mathrm{inr}_{T(a), T(b)} = T(\mathrm{inr}_{a, b})$ по~заданию $\phi$.
