...
\begin{enumerate}
    ...
    \item $f \circ (g \circ h) = (f \circ g) \circ h$, если $(f, g), (g, h) \in A \times_O A$.
    \item $f \circ \mathrm{id}(a) = f$, если $\mathrm{dom}(f) = a$.
    \item $\mathrm{id}(b) \circ f = f$, если $\mathrm{cod}(f) = b$.
\end{enumerate}

Элементы множества $O$ называются \textit{объектами} категории $C = (O, A, \mathrm{dom}, \mathrm{cod}, \mathrm{id}, \circ)$,
а~элементы множества $A$~— \textit{стрелками} этой категории. Обозначим $\mathrm{Ob}(C) = O$ и~$\mathrm{Arr}(C) = A$.
...
такая, что для~всяких объектов $a, b \in \mathrm{Ob}(A)$ и~стрелки $f \in \mathrm{hom}_C(a, b)$ выполняются
следующие условия:
\begin{enumerate}
...
$T_1$ называется \textit{объектной функцией,} а~$T_2$~— \textit{стрелочной.} Всюду далее, по~общепринятому соглашению,
объектную и~стрелочную функции одного функтора будем обозначать одной и~той~же буквой.
