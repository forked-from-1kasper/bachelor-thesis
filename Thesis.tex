...
\begin{titlepage}
  ...
  \begin{tabular}{lL{0.15\textwidth}L{0.35\textwidth}l}
    ...
    Нормоконтролер &  &                                              & Т.~Н.~Шипина \rule{0pt}{28pt}
  \end{tabular}
...
Как~$\Hom(G, H)$ обозначим множество всех гомоморфизмов между группами $G$ и~$H$.
...
Пусть $\diag^{m \times n}_r(x_1, \ldots, x_r)$~— матрица $\diag(x_1, \ldots, x_r)$,
...
\bibitem{Mil65} J.~Milnor. Lectures on~the~H-Cobordism Theorem. Princeton University Press, Princeton (1965).

\bibitem{Stong68} R.~E.~Stong, Notes on~Cobordism Theory. Princeton University Press, 1969.

\bibitem{Weston} T.~Weston, An~introduction to~cobordism theory.
...
Под~\textit{упорядоченным графом} \cite{MacLane71} понимаем упорядоченную четвёртку $(O,\allowbreak A,\allowbreak \dom,\allowbreak \cod)$,
...
\medskip
\noindent\textbf{Пример.} Рассмотрим категорию $n$-мерных $C^p$-гладких многообразий $\mathbf{Manifold}_p^n$ (с~границей),
...
\noindent\textbf{Пример.} Рассмотрим категорию $C^p$-гладких многообразий произвольной размерности $\mathbf{Manifold}_p$,
...
\begin{proof}
    Действительно:
    \begin{enumerate}
        \item Пусть $a \in \Ob(C_1)$ замкнут. $\partial(a)$ инициален, а~потому и~$T(\partial(a)) = \partial(T(a))$.
...
\medskip
\noindent\textbf{Пример.} Для~любой категории $C$ с~инициальным объектом $0$ тройка $(C, \Delta_0, !)$,
очевидно, является категорией с~кобордизмами. Если категория $C$ не~содержит (бинарных) копроизведений,
...
Под~\textit{слабым кобордизмом} (в~\cite{Stong68} называемым просто кобордизмом) между объектами $a$ и~$b$
...
\begin{enumerate}
    ...
    $$
        ...
        \end{cases}
    $$
    \item Слабая кобордантность симметрична; более того, имеем изоморфизм между $\Cob^w(a, b)$ и~$\Cob^w(b, a)$:
    $$
        S = \begin{cases}
        ...
        \end{cases}
    $$
    \item Слабая кобордантность транзитивна \cite{Weston}; действительно, если имеем $(u, v_1) \in \Cob^w(a, b)$ и~$(v_2, w) \in \Cob^w(b, c)$, то $(u + v_2, v_1 + w) \in \Cob^w(a, c)$:
    \begin{align*}
