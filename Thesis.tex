...
выполнено следующее \textit{универсальное свойство:} для~любого объекта $x \in \Ob(C)$ и~пары морфизмов $f \in \hom_C(a, x)$
и~$g \in \hom_C(b, x)$ существует единственный \textit{(универсальный)} морфизм $h \in \hom_C(c, x)$ такой, что $h \circ \inl = f$
