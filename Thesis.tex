...
\bibitem{Stong68} R.~E.~Stong, Notes on~Cobordism Theory. Princeton University Press, 1969.
...
такая, что $f \circ g = \mathrm{id}(b)$ и~$g \circ f = \mathrm{id}(a)$. Тогда также пишем $f : a \cong b$.
...
и~$g \in \mathrm{hom}_C(b, x)$ существует единственный (универсальный) морфизм $h \in \mathrm{hom}_C(c, x)$ такой, что $h \circ \mathrm{inl} = f$
...
Также пишем $c = a + b$, $\mathrm{inl} = \mathrm{inl}_{a, b}$, $\mathrm{inr} = \mathrm{inr}_{a, b}$
и~$h = \mathrm{rec}_{a, b}(f, g)$.

Наконец, говорим, что функтор $T : A \rightarrow B$ \textit{полуаддитивен,} если для~всякого копроизведения $(c, i, j)$ объектов $a$ и~$b \in \mathrm{Ob}(A)$
...
$o \in \mathrm{Ob}(A)$ объект $T(o)$~— инициальный в~$B$.

\begin{lemma*}[$\eta$-правило]
    Если $(c, i, j)$~— копроизведение, $x \in \mathrm{Ob}(C)$ и~$f \in \mathrm{hom}_C(c, x)$,
    то~$\mathrm{rec}(f \circ i, f \circ j) = f$.
\end{lemma*}

\begin{proof}
    Действительно, $\mathrm{rec}(f \circ i, f \circ j) \circ i = f \circ i$
    и~$\mathrm{rec}(f \circ i, f \circ j) \circ j = f \circ j$.
    В~силу единственности такой стрелки получаем лемму.
\end{proof}

\begin{lemma*}
    Пусть $(c, i, j)$~— копроизведение, $x \in \mathrm{Ob}(C)$, $f, g \in \mathrm{hom}_C(c, x)$.
    Если $f \circ i = g \circ i$ и~$f \circ j = g \circ j$, то~$f = g$.
\end{lemma*}

\begin{proof}
    По~доказанному ранее, $\mathrm{rec}(f \circ i, f \circ j) = f$ и~$\mathrm{rec}(g \circ i, g \circ j)$.
    Но~тогда имеем:
    $$
        f = \mathrm{rec}(f \circ i, f \circ j) = \mathrm{rec}(g \circ i, g \circ j) = g.
    $$
\end{proof}
...
\begin{proof}
    Обозначим как $\mathrm{rec}_1$ универсальную стрелку для~первой тройки,
    а~как $\mathrm{rec}_2$~— для~второй. Пусть $\phi = \mathrm{rec}_1(i_2, j_2)$
    и~$\psi = \mathrm{rec}_2(i_1, j_1)$. Тогда:
    $$
        \phi \circ \psi \circ i_2 = \phi \circ \mathrm{rec}_2(i_1, j_1) \circ i_2
                                  = \phi \circ i_1
                                  = \mathrm{rec}_1(i_2, j_2) \circ i_1
                                  = i_2
                                  = \mathrm{id}_C(c_2) \circ i_2.
    $$

    Аналогично, $\phi \circ \psi \circ j_2 = \mathrm{id}_C(c_2) \circ j_2$.
    В~силу предыдущей леммы, $\phi \circ \psi = \mathrm{id}_C(c_2)$.
    Точно так~же получаем, что $\psi \circ \phi = \mathrm{id}_C(c_1)$.
    Наконец, $\phi \circ i_1 = \mathrm{rec}_1(i_2, j_2) \circ i_1 = i_2$
    и~$\phi \circ j_1 = \mathrm{rec}_1(i_2, j_2) \circ j_1 = j_2$.
\end{proof}
...
\begin{theorem*}[критерий полуаддитивности]
    ...
    \begin{enumerate}
        \item Для~всякой пары объектов $a, b \in \mathrm{Ob}(A)$ определён изоморфизм $\phi_{a, b} : T(a) + T(b) \cong T(a + b)$.
        \item $\phi_{a, b} \circ \mathrm{inl}_{T(a), T(b)} = T(\mathrm{inl}_{a, b})$.
        \item $\phi_{a, b} \circ \mathrm{inr}_{T(a), T(b)} = T(\mathrm{inr}_{a, b})$.
    \end{enumerate}
\end{theorem*}

\begin{proof}
    Слева направо: пусть $T$ полуаддитивен. $(a + b, \mathrm{inl}_{a, b}, \mathrm{inr}_{a, b})$~— копроизведение,
    поэтому, по~полуаддитивности, $(T(a + b), T(\mathrm{inl}_{a, b}), T(\mathrm{inr}_{a, b}))$~— тоже.
    Копроизведение единственно, поэтому $\phi : T(a) + T(b) \cong T(a + b)$, причём $\phi \circ \mathrm{inl}_{T(a), T(b)} = T(\mathrm{inl}_{a, b})$
    и~$\phi \circ \mathrm{inr}_{T(a), T(b)} = T(\mathrm{inr}_{a, b})$.

    Обратно, пусть $(c, i, j)$~— некоторое копроизведение $a$ и~$b$. Докажем, что $(T(c), T(i), T(j))$~— копроизведение.
    Зафиксируем объект $x \in \mathrm{Ob}(B)$ и~пару стрелок $f \in \mathrm{hom}_B(T(a), x)$ и~$g \in \mathrm{hom}_B(T(b), x)$.

    Снова в~силу единственности имеем стрелку $\psi : c \cong a + b$.
    Тогда $h = \mathrm{rec}_{T(a), T(b)}(f, g) \circ \phi_{a, b}^{-1} \circ T(\psi) \in \mathrm{hom}_B(T(c), x)$, причём:
    \begin{align*}
        h \circ T(i) &= \mathrm{rec}_{T(a), T(b)}(f, g) \circ \phi_{a, b}^{-1} \circ T(\psi \circ i) \\
                     &= \mathrm{rec}_{T(a), T(b)}(f, g) \circ \phi_{a, b}^{-1} \circ T(\mathrm{inl}_{a, b}) \\
                     &= \mathrm{rec}_{T(a), T(b)}(f, g) \circ \mathrm{inl}_{T(a), T(b)} \\
                     &= f.
    \end{align*}

    Аналогично $h \circ T(j) = g$. Наконец, пусть $h' \in \mathrm{hom}_B(T(c), x)$,
    ...
    \begin{align*}
        h \circ u \circ \mathrm{inl}_{T(a), T(b)} &= \mathrm{rec}_{T(a), T(b)}(f, g) \circ \phi_{a, b}^{-1} \circ T(\psi \circ \psi^{-1}) \circ \phi_{a, b} \circ \mathrm{inl}_{T(a), T(b)} \\
                                                  &= \mathrm{rec}_{T(a), T(b)}(f, g) \circ \mathrm{inl}_{T(a), T(b)} \\
                                                  &= f \\
                                                  &= h' \circ T(i) \\
                                                  ...
                                                  &= h' \circ u \circ \phi_{a,b}^{-1} \circ T(\mathrm{inl}_{a, b}) \\
                                                  &= h' \circ u \circ \mathrm{inl}_{T(a), T(b)}.
    \end{align*}

    Аналогично, $h \circ u \circ \mathrm{inr}_{T(a), T(b)} = h' \circ u \circ \mathrm{inr}_{T(a), T(b)}$, поэтому $h \circ u = h' \circ u$.
\end{proof}
...
\begin{enumerate}
    ...
    \item $\partial(\partial(x))$~— инициальный объект в~$C$ для~всех $x \in \mathrm{Ob}(C)$.
\end{enumerate}
...
$(\mathrm{id}_C(x), 1_F)$ определяет тождественный морфизм для~коконуса $(F, x, \phi)$.
...
\begin{proof}
    ...
    \begin{enumerate}
        \item $(k \mapsto\ !_{\mathrm{colim}(F)})(j) \circ \Delta_0(f) =\ !_{\mathrm{colim}(F)} \circ \mathrm{id}_C(0) =\ !_{\mathrm{colim}(F)} = (k \mapsto\ !_{\mathrm{colim}(F)})(i)$.
        \item ???
        \item ???
    \end{enumerate}
    ...
    \begin{enumerate}
        \item $\partial(\mathrm{id}_{\mathrm{Cocone}(J, C)}((F, x, \phi))) = \partial((\mathrm{id}_C(x), 1_F)) = (\mathrm{rec}(F, \mathrm{colim}(F), \mathrm{incl}(F)), 1_{\Delta_0})$,
        но~поскольку $\mathrm{id}(\mathrm{colim}(F)) \circ \mathrm{incl}_j(F) = \mathrm{incl}_j(F)$, в~силу единственности,
        $\mathrm{rec}(F, \mathrm{colim}(F), \mathrm{incl}(F)) = \mathrm{id}(\mathrm{colim}(F))$. Получаем,
        $\partial(\mathrm{id}((F, x, \phi))) = (\mathrm{id}(\mathrm{colim}(F)), 1_{\Delta_0}) = \mathrm{id}(\partial((F, x, \phi)))$.
...
Говорим, что объект $a \in \mathrm{Ob}(C)$ \textit{замкнут,} если $\partial(a)$~— инициальный объект.
Объект $a$ \textit{точен,} если $\exists b \in \mathrm{Ob}(C){:}\ a = \partial(b)$.
...
\begin{proof}
    Пусть $a \in \mathrm{Ob}(C_1)$ замкнут. $\partial(a)$ инициален, а~потому и~$T(\partial(a)) = \partial(T(a))$.

    Пусть $a = \partial(b)$. Тогда $T(a) = T(\partial(b)) = \partial(T(b))$.

    Пусть $(c, i, j)$~— кобордизм между $a$ и~$b \in \mathrm{Ob}(C_1)$.
    Тогда $T(i) \in \mathrm{hom}_{C_2}(T(a), T(\partial(c))) = \mathrm{hom}_{C_2}(T(a), \partial(T(c)))$
    и~$T(j) \in \mathrm{hom}_{C_2}(T(b), T(\partial(c))) = \mathrm{hom}_{C_2}(T(b), \partial(T(c)))$.
