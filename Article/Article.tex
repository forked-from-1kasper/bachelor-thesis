...
\begin{proof}
    ...
    $
        \varepsilon = \diag^{n \times k}_r(\varepsilon_1, \ldots, \varepsilon_r).
    $
...
Действительно, заметим, что группа $\image(\phi_\alpha)$ изоморфна группе $(K^\times)^r$. В~доказательстве леммы \ref{lemma:kernelAsAnImage} мы видели, что для~всякого $\phi_\alpha$
